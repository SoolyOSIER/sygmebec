...
import { FiPlus, FiUser, FiRefreshCw, FiTrash2, FiSearch, FiX, FiLock } from 'react-icons/fi'
import {
      ...
      render: (value, item) => (
        <div className="flex items-center justify-end gap-1">
          <Button
            ...
            size="sm"
            icon={FiRefreshCw}
            onClick={() => { setResetTarget(item); setResetError('') }}
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal
        ...
        title={`Réinitialiser le mot de passe — ${resetTarget?.identifiant || ''}`}
        size="sm"
      >
        <form onSubmit={handleResetPassword} className="space-y-5">
          <p className="text-sm text-secondary-500">Saisissez et confirmez le nouveau mot de passe.</p>
          <Input label="Nouveau mot de passe" type="password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} required icon={FiLock} />
          <Input label="Confirmer le mot de passe" type="password" value={newPasswordConfirm} onChange={(event) => setNewPasswordConfirm(event.target.value)} required icon={FiLock} error={resetError} />
          <div className="flex justify-end gap-3 border-t border-gray-100 pt-4">
            <Button type="button" variant="ghost" onClick={closeResetModal}>Annuler</Button>
            <Button type="submit" variant="success" icon={FiLock} isLoading={isResetting}>Réinitialiser</Button>
          </div>
        </form>
      </Modal>
    </div>

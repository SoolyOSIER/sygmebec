...
import { FiActivity, FiCheck, FiDownload, FiEdit2, FiEye, FiFileText, FiImage, FiMail, FiPhone, FiPlus, FiSearch, FiStar, FiTrash2, FiUserCheck, FiUsers, FiUserX } from 'react-icons/fi'
...
const relativeDate = (value, tr) => { if (!value) return tr('members.dateNotSpecified'); const days = Math.max(0, Math.floor((Date.now() - new Date(value).getTime()) / 86400000)); return days === 0 ? tr('members.today') : tr('members.daysAgo', { count: days }) }
...
export default function MembresListPage() {
  ...
  const { data: exportMembersData } = useMembres({ page_size: 1000 })
  ...
  const exportableMembers = list(exportMembersData)
  ...
  const groupCards = groups.length ? groups : [t('members.worship'), t('members.welcome'), t('members.youth'), t('members.intercession')].map((name, index) => ({ name, count: 0, tone: ['green', 'blue', 'purple', 'amber'][index] }))
  ...
  const exportMembers = async (format) => {
    if (!exportMemberId) {
      toast.error('Sélectionnez le membre à exporter.')
      return
    }
    setExporting(true)
    try {
      const response = await membresApi.exportMembres(format, {
        membre: exportMemberId,
      })
      const extension = format === 'xlsx' ? 'xlsx' : format
      const member = exportableMembers.find((item) => String(item.id) === String(exportMemberId))
      const memberName = member?.nom_complet || `${member?.nom || ''} ${member?.prenom || ''}`.trim()
      const safeName = (memberName || `membre-${exportMemberId}`).normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-zA-Z0-9]+/g, '-').replace(/^-|-$/g, '').toLowerCase()
      const link = document.createElement('a')
      const url = URL.createObjectURL(response.data)
      link.href = url
      link.download = `fiche-membre-${safeName}.${extension}`
      link.click()
      URL.revokeObjectURL(url)
      setExportOpen(false)
    } catch {
      toast.error('Impossible de générer l’export des membres.')
    } finally {
      setExporting(false)
    }
  }
  ...
  return <div className="members-reference">
    <header className="members-top-search"><div className="members-search"><FiSearch /><input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1) }} placeholder={t('management.searchMembers')} /></div></header>
    <div className="members-content">
      <div className="members-page-head"><div><span className="members-eyebrow">Gestion des membres</span><h1>Membres de l’assemblée</h1><p>Consultez, filtrez et gérez la fiche de chaque membre enregistré.</p></div><div className="members-head-actions"><div className="members-export"><button type="button" className="members-button" disabled={exporting} onClick={() => setExportOpen((open) => !open)}><FiDownload />{exporting ? 'Exportation…' : 'Exporter'}</button>{exportOpen && <div className="members-export-menu"><label>Choisir le membre<select value={exportMemberId} onChange={(event) => setExportMemberId(event.target.value)}><option value="">Sélectionner un membre</option>{exportableMembers.map((member) => <option key={member.id} value={member.id}>{member.nom_complet || `${member.nom || ''} ${member.prenom || ''}`.trim()}</option>)}</select></label><button type="button" disabled={!exportMemberId} onClick={() => exportMembers('xlsx')}><FiDownload />Excel (.xlsx)</button><button type="button" disabled={!exportMemberId} onClick={() => exportMembers('pdf')}><FiFileText />PDF</button><button type="button" disabled={!exportMemberId} onClick={() => exportMembers('png')}><FiImage />Image (.png)</button></div>}</div><Link to="/membres/nouveau" className="members-button members-primary"><FiPlus />Ajouter un membre</Link></div></div>
      ...
      <section className="members-overview">
        ...
        <article className="members-panel"><header><div><h3>Activité récente</h3><p>Derniers mouvements dans le registre des membres</p></div><span>7 derniers jours</span></header><div className="members-feed">{recentMembers.length ? recentMembers.map((member, index) => <div key={member.id}><i className={['green', 'purple', 'blue', 'amber'][index]}>{index === 0 ? <FiPlus /> : index === 1 ? <FiStar /> : <FiEdit2 />}</i><p><b>{`${member.prenom || ''} ${member.nom || ''}`.trim() || tr('members.member')}</b> {index === 0 ? `a été ajouté au registre — ${tr('members.member')} #${member.id}` : 'a mis à jour sa fiche membre'}<small>{relativeDate(member.created_at || member.date_adhesion, tr)}</small></p></div>) : <p className="members-empty">Aucune activité récente.</p>}</div></article>
      </section>

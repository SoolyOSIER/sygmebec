...
export default function RapportsListPage() {
  ...
  return <div className="programme-page reports-page">
    <div className="programme-search"><FiSearch /><input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1) }} placeholder={t('management.searchReports')} /></div>
    <div className="programme-head"><div><span>{t('navigation.reports')}</span><h1>{t('management.reportsTitle')}</h1><p>{t('management.reportsDescription')}</p></div><div><button type="button" className="programme-button"><FiFilter />Filtrer</button><Link to="/rapports/generer" className="programme-button primary"><FiPlus />{t('management.generateReport')}</Link></div></div>
    ...
    <section className="programme-stats">{[[t('management.total'), reports.length, FiFileText, 'blue', t('management.generated')], [t('management.available'), available, FiDownload, 'green', t('management.download')], [t('management.pending'), pending, FiClock, 'amber', t('management.pending')], [t('management.failed'), failed, FiAlertTriangle, 'purple', t('management.failed')]].map(([label, value, Icon, color, note]) => <article key={label} className={color}><i><Icon /></i><strong>{value}</strong><p>{label}</p><small>{note}</small></article>)}</section>

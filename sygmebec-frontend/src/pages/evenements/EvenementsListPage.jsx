...
const formatDate = (value) => formatLocalizedDate(value)
const formatEventDate = (value, locale) => {
  if (locale !== 'ht-HT') return new Intl.DateTimeFormat(locale, { dateStyle: 'medium' }).format(new Date(value))
  const date = new Date(value)
  return `${date.getDate()} ${creoleMonths[date.getMonth()]} ${date.getFullYear()}`
}
...
export default function EvenementsListPage() {
  ...
  const exportEvents = () => {
    ...
    const values = events.map((event) => [event.titre, formatEventDate(event.date, locale), event.lieu, fullName(event), status(event.date, t)[1]])
  ...
  return <div className="programme-page events-reference">
    <div className="programme-search"><FiSearch /><input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1) }} placeholder={t('management.searchEvents')} /></div>

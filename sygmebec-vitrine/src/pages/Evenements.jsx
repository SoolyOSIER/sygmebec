...
export default function Evenements() {
  ...
  return <><SEO title={labels.title} description={labels.description} /><main className="relative overflow-hidden py-16 md:py-20"><div className="absolute inset-x-0 top-0 -z-10 h-96 bg-gradient-to-b from-primary-50 via-white to-transparent" /><div className="container-custom"><AnimatedSection><header className="mx-auto max-w-3xl text-center"><span className="section-subtitle"><CalendarDays className="h-4 w-4" /> {labels.events}</span><h1 className="section-title mt-4">{labels.subtitle}</h1><p className="mt-4 text-lg leading-relaxed text-gray-600">{labels.description}</p><p className="mt-6 inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-semibold text-primary-700 shadow-sm"><Sparkles className="h-4 w-4 text-gold-600" /> {evenements.length} {labels.event}{evenements.length > 1 ? 's' : ''} {labels.published}{evenements.length > 1 ? 's' : ''}</p></header></AnimatedSection><AnimatedSection delay={0.08}><div className="relative mx-auto mt-10 max-w-2xl"><Search className="absolute left-5 top-1/2 h-5 w-5 -translate-y-1/2 text-primary-600" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={labels.search} className="w-full rounded-2xl border border-slate-200 bg-white py-4 pl-14 pr-5 text-slate-900 shadow-sm outline-none transition focus:border-primary-400 focus:ring-4 focus:ring-primary-100" /></div></AnimatedSection>{loading ? <p className="py-20 text-center text-gray-500">{labels.loading}</p> : failed ? <div className="py-20 text-center"><p className="text-rose-700">{labels.error}</p></div> : results.length === 0 ? <div className="py-20 text-center"><CalendarDays className="mx-auto mb-4 h-14 w-14 text-slate-300" /><p className="text-lg font-semibold text-navy-900">{labels.empty}</p><p className="mt-2 text-sm text-slate-500">{labels.tryAgain}</p></div> : <section className="mt-12 grid grid-cols-1 gap-7 md:grid-cols-2 xl:grid-cols-3">{results.map((event, index) => <EvenementCard key={event.id} evenement={event} index={index} />)}</section>}</div></main></>
}
